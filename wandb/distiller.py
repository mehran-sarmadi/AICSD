...
class Distiller(nn.Module):
    def __init__(self, t_net, s_net, t_vit, s_vit, args):
    ...
    def forward(self, x):
        ...
        # t_feats len = 6 s_feats len = 6
        print('teacher model:')
        print(f"t_feats[0] shape = {t_feats[0].shape} t_feats[1] shape = {t_feats[1].shape} t_feats[2] shape = {t_feats[2].shape} t_feats[3] shape = {t_feats[3].shape} t_feats[4] shape = {t_feats[4].shape} t_feats[5] shape = {t_feats[5].shape}")
        print('\n\nstudent model:')
        print(f"s_feats[0] shape = {s_feats[0].shape} s_feats[1] shape = {s_feats[1].shape} s_feats[2] shape = {s_feats[2].shape} s_feats[3] shape = {s_feats[3].shape} s_feats[4] shape = {s_feats[4].shape} s_feats[5] shape = {s_feats[5].shape}")
        print('\n\noutputs:')
        print(f"t_out shape = {t_out.shape} s_out shape = {s_out.shape}")
        # print(f"t_feats len = {len(t_feats)} s_feats len = {len(s_feats)} t_out shape = {t_out.shape} s_out shape = {s_out.shape}")
        # print(f' t_feats_type = {type(t_feats)} s_feats_type = {type(s_feats)} t_out_type = {type(t_out)} s_out_type = {type(s_out)}')
        ...
        vit_loss = 0
        if self.args.vit_lambda is not None:
           t_vit_output = self.t_vit(t_out)
           s_vit_output = self.s_vit(s_out)
           cross_entropy = nn.CrossEntropyLoss()
           mse = nn.MSELoss()
           vit_loss = self.args.vit_lambda * mse(s_vit_output, t_vit_output)
        print(f'vit loss = {vit_loss}')
        return s_out, pi_loss, vit_loss
