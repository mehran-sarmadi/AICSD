...
from vit_pytorch import DistillerViT
...
class Trainer(object):
    def __init__(self, args):
       ...
       # checkpoint = torch.load('/kaggle/working/model.pth.tar')
        ...
        self.d_net = distiller.Distiller(self.t_net, self.s_net, self.t_vit, self.s_vit, self.args)
       ...
       # print('Teacher Net: ')
        ...
        print('the number of teacher vit model parameters: {}'.format(
            sum([p.data.nelement() for p in self.t_vit.parameters()])))
        print('the number of student vit model parameters: {}'.format(
            sum([p.data.nelement() for p in self.s_vit.parameters()])))
        ...
        distill_params = [{'params': self.s_net.get_1x_lr_params(), 'lr': args.lr},
                          ...
                          {'params': self.d_net.t_vit.parameters(), 'lr': args.lr * 10},
                          {'params': self.d_net.s_vit.parameters(), 'lr': args.lr * 10}]


        init_params = [{'params': self.d_net.t_vit.parameters(), 'lr': args.lr},
                        {'params': self.d_net.s_vit.parameters(), 'lr': args.lr}]
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            output, pi_loss, vit_loss = self.d_net(image)
            # print(self.d_net)
            ...
            loss = loss_seg + pi_loss + vit_loss 
...
def main():
    ...
    parser.add_argument('--vit_lambda', type=float, default=None,
                        help='coefficient for vit loss')
